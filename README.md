...
The scheduled workers run only from a published Netlify production deploy. Their service-role and Resend keys are Functions-only protected variables; they must never be copied into `.env.local`, GitHub Actions, preview logs, or browser code. A release is not operationally complete until every registered worker has produced a successful live run or an explicitly safe disabled skip, and `/api/health` reports each one within its stale threshold.
