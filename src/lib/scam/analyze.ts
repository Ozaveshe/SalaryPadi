...
const FLAG_DEFINITIONS: Record<ScamFlagCode, FlagDefinition> = {
  upfront_payment: {
    severity: "high",
...
function evidenceSnippet(value: string): string {
...
function feeIsNegated(statement: string): boolean {
  return (
...
function safetyWarningIsNegated(statement: string): boolean {
  return (
...
function isSameOrSubdomain(candidate: string, expected: string): boolean {
...
function couldBeLookalike(candidate: string, official: string): boolean {
...
function extractUrls(statements: readonly string[]): ExtractedUrl[] {
