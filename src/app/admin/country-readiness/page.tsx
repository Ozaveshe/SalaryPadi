...
import { getCountryPackReadiness } from "@/lib/operations/country-pack-readiness";
...
export default async function CountryReadinessPage() {
  await requireAdmin();
  const readiness = await getCountryPackReadiness();
  ...
  return (
    <div className="stack-lg">
            ...
            {readiness.countries.map((country) => (
              <tr key={country.country_code}>
                ...
                <td>
                  ...
                  <span>
                    target{" "}
                    {percentage(country.thresholds.explicit_eligibility_ratio)}
                  </span>
                </td>
                <td>
                  <span>
                ...
                <td>
                  <span className="status status-neutral">
                    {country.activation_ready ? "Ready for review" : "Blocked"}
                  </span>
                  <span>
      ...
      <p className="muted">
        “Ready for review” is not activation. An administrator with AAL2 must
        record the review and explicitly enable routes; deployment remains a
        separate approval.
      </p>
    </div>
  );
