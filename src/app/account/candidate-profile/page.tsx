...
import { CvPanel } from "@/components/career/cv-panel";
...
import { readCvDraft } from "@/lib/career/cv/draft";
...
export default async function CandidateProfilePage({
  ...
  const { status, cv: cvStatus } = await searchParams;
  const [result, cvs] = await Promise.all([
    getCandidateProfile(),
    getCandidateCvs(),
  ]);
  const profile = result.data;
  const currentCv = cvs.data.find((row) => row.is_current) ?? null;
  const draft =
    currentCv?.extracted_text && currentCv.parse_state === "parsed"
      ? readCvDraft(currentCv.extracted_text)
      : null;

  /*
   * A CV proposal only fills a field the owner has not answered themselves.
   * A saved answer is an attested claim and a reading off a document is not, so
   * the reading never overwrites one — it only offers something where there was
   * nothing. The owner still has to save before any of it counts.
   */
  const proposed = {
    headline: profile?.headline ?? draft?.headline?.value ?? "",
    yearsExperience:
      profile?.years_experience ?? draft?.yearsExperience?.value ?? null,
    experienceLevel:
      profile?.experience_level && profile.experience_level !== "unspecified"
        ? profile.experience_level
        : (draft?.experienceLevel?.value ?? "unspecified"),
    locationCountry:
      profile?.location_country ?? draft?.locationCountry?.value ?? "",
  };
  const usedDraft =
    draft !== null &&
    (proposed.headline === draft.headline?.value ||
      proposed.yearsExperience === draft.yearsExperience?.value ||
      proposed.experienceLevel === draft.experienceLevel?.value ||
      proposed.locationCountry === draft.locationCountry?.value);

  return (
    <div className="site-shell">
      <WorkspaceShell
        unreadNotifications={await readUnreadNotificationCount()}
        current="/account/candidate-profile"
      ...
      >
        {status === "saved" ? (
          ...
          <div className="notice notice-danger" role="alert">
            The profile could not be saved. Check the fields and try again. A
            pay expectation needs both a currency and a pay period.
          </div>
        ) : null}
        ...
        {result.state === "unconfigured" ? (
          ...
          <>
            <CvPanel cvs={cvs.data} draft={draft} status={cvStatus} />

            {usedDraft ? (
              <div className="notice notice-warning" role="status">
                Some fields below are filled in from your CV and are{" "}
                <strong>not saved yet</strong>. Check every one, correct
                anything that is wrong, then save — that save is what makes them
                your own stated claims.
              </div>
            ) : null}
            ...
            >
              ...
              <p className="text-muted m-0">
                ...
                optional — anything you leave blank is treated as not stated,
                and it lowers how much of the comparison can be made rather than
                counting against you.
              </p>
              <p className="text-muted m-0 text-sm">
                The score does not compare skills, it is not an assessment of
                your suitability, and it does not predict whether you will be
                hired. Only an employer decides that.
              </p>
            </section>
            ...
            >
              <fieldset className="stack">
                ...
                <div className="form-grid">
                  <div className="field">
                    ...
                    <input
                      className="input"
                      defaultValue={proposed.headline}
                      id="headline"
                      maxLength={160}
                      name="headline"
                      type="text"
                  ...
                  <div className="field">
                    ...
                    <input
                      className="input"
                      defaultValue={
                        proposed.yearsExperience === null
                          ? ""
                          : String(proposed.yearsExperience)
                      }
                      id="years_experience"
                      inputMode="numeric"
                      max={60}
                  ...
                  <div className="field">
                    ...
                    <select
                      className="select"
                      defaultValue={proposed.experienceLevel}
                      id="experience_level"
              ...
              <fieldset className="stack">
                ...
                <p className="field-help m-0">
                  ...
                  posting. SalaryPadi never converts between currencies here, so
                  a role quoted in another currency is reported as not compared
                  rather than guessed.
                </p>
                <div className="form-grid">
                  <div className="field">
