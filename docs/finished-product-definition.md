...
What "finished" means for SalaryPadi, and where each part stands as of the
9 August 2026 audit ([product-completion-audit.md](product-completion-audit.md)).
...
| Career-data associate              | Partial — queues exist but admin-only, one generic table         | No job detail view, duplicate queue unreachable, no operator intake           |
| Moderator                          | Partial — complete immutable DB layer                            | UI shows none of the nine flag signals; moderator role cannot log in usefully |
...
staff roles below admin reaching their workspaces.
