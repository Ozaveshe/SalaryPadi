...
| 5   | A contribution is identifiable from an aggregate   | Real-world consequences for the contributor                                          | Per-metric thresholds, 24-hour lag, shape suppression wired into every aggregate worker that exists                           | Low — no worker publishes without it; interview aggregates have a rule row and no worker     |
...
| 7   | An employer edits independent evidence             | Trust data becomes marketing                                                         | No write grant for any signed-in account; every employer write is a definer RPC into moderation; declared write-path registry | Low — no employer profile editor exists; adding one fails CI until its fields are classified |
