...
| Jobicy public API                                             | Active; official API fair-use terms re-reviewed 2026-08-13, next review due 2026-09-13                                       | Yes, with visible Jobicy attribution                                      | Bounded metadata snapshot only; no full description                                                 | No                                         | The returned Jobicy URL                  |
| Himalayas public jobs API                                     | Active; official backfill and attribution terms re-reviewed 2026-08-13, next review due 2026-09-13                           | Yes, with visible Himalayas attribution                                   | Bounded metadata snapshot only; no full description                                                 | No; no downstream job-platform syndication | The returned Himalayas URL               |
| ReliefWeb jobs API                                            | Active with an approved server-side appname; policy review due 2026-08-26                                                    | Metadata-only listings with ReliefWeb and information-partner attribution | One replaceable bounded snapshot; no full description                                               | No                                         | The returned ReliefWeb URL               |
...
| Reviewed employer ATS registry                                | Enabled board by board only after a current policy, exact tenant configuration, destination allowlist and country right pass | Yes, only for the country packs granted to that source                    | Bounded source records and canonical jobs under each source's retention policy                      | Only where the country right permits it    | Employer careers or ATS application page |
| Wave, GiveDirectly, ALX Africa and Educate! Greenhouse boards | Active; public Job Board API and employer careers surfaces reviewed 2026-08-12                                               | Nigeria-rights projection only; other country-pack roles remain held      | Full description permitted with 30-day raw retention                                                | Yes for the active Nigeria right           | Employer-owned destination allowlist     |
...
The repository contains a disabled-by-default Greenhouse, Lever, and Ashby adapter contract under `src/lib/jobs/ats/`, normalization and quarantine logic in `src/lib/jobs/ats-import.ts`, and the gated `netlify/functions/ats-source-sync.mts` worker. Those files are infrastructure, not permission. A named source becomes callable only through separately reviewed production policy, private tenant configuration, destination allowlists, dependencies and country rights. The checked registry in `config/employer-board-registry.json` and applied data scripts under `docs/data/` retain the non-secret evidence for named sources; neither changes the default-deny behavior for any unregistered board.
