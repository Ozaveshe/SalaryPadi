...
- Cache public responses for 43,200 seconds (twelve hours). At 01:05 and 13:05 UTC the scheduled worker reads or revalidates that same cache entry through a protected internal route, then derives the alert snapshot from it. A database claim is required on every cache miss, with at most one provider request per minute and four in any rolling 24-hour window. A failed refresh does not deliberately evict a still-valid last-known-good snapshot.
...
Terms basis: the official [Remotive public API repository](https://github.com/remotive-com/remote-jobs-api) and [API page](https://remotive.com/remote-jobs/api), reviewed on 2026-07-10. The specific API material permits low-frequency attributed sharing, but Remotive's newer [general terms](https://remotive.com/terms-of-use) and current [robots policy](https://remotive.com/robots.txt) create a policy ambiguity. The pilot must remain low-frequency, attributed, noindex, and immediately disableable while the source owner obtains written confirmation before treating it as a core commercial source.
