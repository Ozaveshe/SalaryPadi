...
- Production responses carry `Strict-Transport-Security: max-age=31536000; includeSubDomains`. Development is deliberately excluded so a loopback server cannot pin `localhost` to HTTPS for unrelated projects. `preload` is not sent: submission to the browser preload list is a separate and effectively irreversible decision.
- Every page path that requires a session is declared once in `src/lib/security/protected-paths.ts`. The request proxy enforces it, `next.config.ts` derives the `private, no-store` cache rules from it, and `robots.txt` derives its disallow list from it, so a newly protected route cannot be left crawlable or cacheable.
- Runtime dependencies carry no known advisories. `npm audit` reports high-severity findings for `brace-expansion` (GHSA-mh99-v99m-4gvg) reached only through `eslint`/`eslint-config-next`. These are development-only, never bundled, and are driven solely by this repository's own lint globs. They are not fixable in place: `minimatch@3` requires `brace-expansion@1`, so overriding to the patched `5.0.8` breaks linting outright, and `npm audit fix --force` proposes a major downgrade of `eslint-config-next`. Re-check when `eslint-config-next` next moves to a `minimatch@9`+ chain.
...
- Tracked workers cover source validation, alert delivery, currency references, expiry, retention, and aggregate maintenance. A production log sink and dedicated external incident-paging integration are still not configured. The secretless scheduled GitHub freshness workflow now checks the public health surface and key routes every six hours, with GitHub's repository-owner workflow-failure email as the baseline alert; investigate failures in Netlify/Supabase inside the documented stale thresholds.
