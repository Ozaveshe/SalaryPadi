...
The worker prefers AfroTools' strict `X-AfroTools-Catalog-ETag` SHA-256 mirror. If an intermediary omits that mirror, it accepts only a bounded printable strong or weak standard HTTP `ETag` as an opaque validator after validating the complete response body. It persists the selected value exactly and sends it as `If-None-Match` on the next run. For a strict AfroTools validator it also sends `X-AfroTools-If-None-Match`, because an intermediary may consume or normalize the standard conditional header before the function receives it. A valid `304` refreshes the snapshot check time without replacing verified metadata; a missing or mismatched validator is rejected. The key, digest and provider response bodies are never logged.
