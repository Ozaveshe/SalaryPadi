...
| Single connector interface           | Feeds emit `AtsSourceRecord` and reuse `normalizeAtsImportRecords` — one canonical pipeline, no second path. DB store for feed sources additionally needs a provider-constraint migration at first authorization (documented deviation: not migrated speculatively)               |
