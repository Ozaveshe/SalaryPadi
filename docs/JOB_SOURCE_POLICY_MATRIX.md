...
| Employer-authorized Greenhouse boards   | Employer ATS     | Template disabled; three reviewed boards (Moniepoint, Canonical, Zipline) active via per-board registration | Written permission or commercial contract from each employer; official documented Job Board API is transport documentation, not republication permission                                                                                                                                         | No board is allowlisted                                   | No by default    | Employer/source specific                               | Every two hours, deterministic 17-minute jitter; contract may slow it       | No until per-board approval           | No until per-board approval | No until per-board approval | Exact tenant allowlist and written employer permission                         |
...
| Employer-authorized XML/JSON feeds | Direct employer | Disabled template | Written per-feed authorization from the employer for a feed the employer controls; the connector (src/lib/jobs/feeds) refuses any record whose destination leaves the feed's allowlisted employer hosts. | Feed-registry-mapped fields only | No by default | Employer-specific | Four fetches/day per feed proposed; retention per feed authorization | No until per-feed authorization | No until per-feed authorization | No until per-feed authorization | Written employer permission, employer-feed-registry entry, destination host allowlist |

| Employer CSV imports | Direct employer | Disabled template | Written per-employer import authorization; uploads accepted only from an authorized employer contact and mapped through the same feed registry and destination allowlist. | Feed-registry-mapped fields only | No by default | Employer-specific | Event-driven uploads; retention per authorization | No until per-employer authorization | No until per-employer authorization | No until per-employer authorization | Written employer permission, employer-feed-registry entry, destination host allowlist |

| Employer-authorized SmartRecruiters boards | Employer ATS | Disabled template | The public Posting API is transport documentation, not republication permission; probing found zombie boards with year-old postings (freshness gate mandatory before any registration). | No board is allowlisted | No by default | Employer/source specific | Every two hours proposed | No until per-board approval | No until per-board approval | No until per-board approval | Exact tenant allowlist, written employer permission, board freshness probe |

| Jooble partner API | Licensed partner | Disabled | Jooble grants API keys by application; its storage and republication terms have not been obtained or reviewed. No request may be made until the reviewed terms are recorded here. | None | No | Unresolved | None until terms exist | No | No | No | Partner API key, republication terms review, field and retention schedule |
